...
function HomeAfterProfile({ onNav, onUpdateProfile, showUpdatePrompt = true }) {
  const [dismissedBenefit, setDismissedBenefit] = useState(false);
  return (
    <div className="screen">
      <div className="header">
        <div className="header-title"><span className="brand-pru">PRU</span><span className="brand-twin">Twin</span></div>
      ...
      <div className="scrollarea" style={{ padding: "0 16px 24px" }}>
        {showUpdatePrompt && !dismissedBenefit && (
          <div className="card" style={{ background: "var(--red-tint-05)", borderLeft: "4px solid var(--red)", marginTop: 4, marginBottom: 12, position: "relative" }}>
            <span onClick={() => setDismissedBenefit(true)} style={{ position: "absolute", top: 10, right: 12, cursor: "pointer", color: "var(--gray-text)" }}>✕</span>
            <div style={{ display: "flex", gap: 10 }}>
              <span style={{ fontSize: 18 }}>🔔</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 13.5, lineHeight: 1.4 }}>
                ...
                <button className="pill-btn-outline-red" style={{ marginTop: 10 }} onClick={() => onNav("profile")}>Xem Hồ sơ →</button>
              </div>
            </div>
          </div>
        )}
