...
/* =========================================================
   ...
   ========================================================= */
function HomeAfterProfile({ onNav, onUpdateProfile, showUpdatePrompt = true, showBenefitBanner = true, onGoProfileFromBanner }) {
  return (
    <div className="screen">
      <div className="header">
        <div className="header-title"><span className="brand-pru">PRU</span><span className="brand-twin">Twin</span></div>
      ...
      <div className="scrollarea" style={{ padding: "0 16px 24px" }}>
        {showBenefitBanner && (
          <div className="card" style={{ background: "var(--red-tint-05)", borderLeft: "4px solid var(--red)", marginTop: 4, marginBottom: 12, position: "relative" }}>
            <span onClick={onGoProfileFromBanner} style={{ position: "absolute", top: 10, right: 12, cursor: "pointer", color: "var(--gray-text)" }}>✕</span>
            <div style={{ display: "flex", gap: 10 }}>
              <span style={{ fontSize: 18 }}>🔔</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 13.5, lineHeight: 1.4 }}>
                ...
                <button className="pill-btn-outline-red" style={{ marginTop: 10 }} onClick={onGoProfileFromBanner}>Xem Hồ sơ →</button>
              </div>
            </div>
          </div>
        )}
        {!showBenefitBanner && showUpdatePrompt && (
          <div className="card attn-card" style={{ borderLeft: "5px solid var(--red)", marginTop: 4 }}>
            <div style={{ display: "flex", gap: 10 }}>
              <span style={{ position: "relative", fontSize: 18 }}>
                ✏️
                <span className="attn-dot">
                  <span className="attn-dot-ping" />
                  <span className="attn-dot-core" />
                </span>
              </span>
              <div>
...
function UpdateProfileScreen({ onBack, onNext }) {
  ...
  // AI tự động chọn đáp án gợi ý sau ~1s, rồi tự điền chi tiết
  // (giống hiệu ứng auto-fill ở Layer 2/3 của luồng tạo hồ sơ)
  useEffect(() => {
    const t = setTimeout(() => setSelected("Gia đình"), 1000);
    return () => clearTimeout(t);
  }, []);

  useEffect(() => {
    if (selected === "Gia đình") {
      const t = setTimeout(() => setDetail("Đón đứa con đầu lòng, có thêm thành viên mới"), 700);
      return () => clearTimeout(t);
    }
  }, [selected]);
