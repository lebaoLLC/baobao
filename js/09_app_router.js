/* =========================================================
   ...
   chatSim          | Frame 4 (Mô phỏng tình huống) +      | 08_chat_ai_flow.js
                    | Frame 5 (Xác nhận hiểu quyền lợi,    |
                    | nối tiếp ngay trong cùng màn chat)   |
   chatContractActive | Frame 6 (Hợp đồng chính thức       | 08_chat_ai_flow.js
                    | có hiệu lực, sau 21 ngày)             |
   ---------------------------------------------------------------
   ========================================================= */
...
function App() {
  ...
  // true sau khi user đã bấm "Xem Hồ sơ →" từ banner "3 quyền lợi sức khỏe" ở Trang chủ.
  // Trước đó: chỉ hiện banner quyền lợi. Sau đó: banner quyền lợi ẩn đi, thay bằng banner "Cập nhật PRUTwin".
  const [benefitBannerSeen, setBenefitBannerSeen] = useState(false);
  ...
  const handleBottomNav = (key) => {
    ...
    else if (key === "prutwin") setRoute(hasProfile ? "chatWelcome" : "onboardWelcome");
    else if (key === "adjust") setRoute(hasProfile ? "adjustAfter" : "adjustPrompt");
    else if (key === "more") setRoute("more");
    else if (key === "notifications") setRoute("notifications");
  };
  ...
  switch (route) {
    ...
    case "home2":
      content = (
        <HomeAfterProfile
          ...
          onUpdateProfile={() => setRoute("updateProfile")}
          showUpdatePrompt={!profileUpdated}
          showBenefitBanner={!benefitBannerSeen}
          onGoProfileFromBanner={() => { setBenefitBannerSeen(true); setRoute("profileAfter"); }}
        />
      );
      break;
    ...
    case "chatQA":
      ...
      break;
    case "chatSim":
      content = <ChatSimulation onBack={() => setRoute(hasProfile ? "home2" : "home1")} onContractActive={() => setRoute("chatContractActive")} />;
      break;
    case "chatContractActive":
      content = <ChatContractActive onBack={() => setRoute(hasProfile ? "home2" : "home1")} />;
      break;
